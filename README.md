...
The pipeline flows left to right, with a pipeline register latching values
between every pair of stages:

![Pipelined MIPS datapath with hazard detection and forwarding](docs/pipeline_datapath.svg)

*Five stages (IF, ID, EXE, MEM, WB) separated by the IF/ID, ID/EXE, EXE/MEM, and MEM/WB pipeline registers. The hazard detection unit stalls the PC and IF/ID and flushes ID/EXE on a load-use hazard, while the forwarding unit routes results from EXE/MEM and MEM/WB back to the ALU inputs.*
